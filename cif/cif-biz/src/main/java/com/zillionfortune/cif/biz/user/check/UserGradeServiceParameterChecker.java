 ...
 */
@Component
public class UserGradeServiceParameterChecker {
	...
	public void checkUpdateGradeRequest(UserGradeServiceRequest request) throws Exception {
		...
		if (null == PersonGradeType.getEnum(Integer.valueOf(request.getGradeType()))) {
			throw new BusinessException("gradeType 字段的参数值不在约定的范围");
		}
		if (request.getGrade().length() != 1) {
			throw new BusinessException("grade 字符长度超出");
		}
		if (request.getGradeType().equals(String.valueOf(PersonGradeType.AUTH.getCode())) && 
				null == AuthGrade.getEnum(request.getGrade())) {
